...
def plot_predictions_vs_ground_truth(pred, ground_truth, batch_x, num_samples=20, channel=0, start=0, save_path="/home/mtiomoko/post_training_forecasting_official-main/plots/prediction_vs_truth.png"):
    """
    Plots predictions vs. ground truth for a given channel (e.g., channel 0) and for the first num_samples.
    If save_path is provided, saves the plot image to that path.
    """
    ...
    for i in range(start, start + num_samples):
        ...
        ax.plot(np.arange(len(batch_x[i, :, channel])), batch_x[i, :, channel], color='g', linestyle='-', linewidth=2, label="Context")
        
        # Plot prediction and ground truth
        ax.plot(np.arange(forecast_start, forecast_start + len(pred[i, :, channel])), pred[i, :, channel], color='b', linestyle='--', linewidth=2, label="Prediction")
        ax.plot(np.arange(forecast_start, forecast_start + len(ground_truth[i, :, channel])), ground_truth[i, :, channel], color='r', linewidth=2, label="Ground Truth")
        
        # Vertical line to separate context and prediction
    ...
    # Show plot in Streamlit (optional)
    st.pyplot(fig)

    # Close the figure to free memory (especially useful in batch inference)
    plt.close(fig)