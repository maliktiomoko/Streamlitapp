...
class Args:
    def __init__(self, **kwargs):
        ...
        self.exog_dim = kwargs.get('exog_dim', 1)
        ...
        self.root_path = kwargs.get('root_path', "/home/mtiomoko/src/post_training_forecasting_official/data/")  # XXX
        self.features = kwargs.get('features', "M")
        self.target = kwargs.get('target', "OT")
        self.freq = kwargs.get('freq', "h")
        self.checkpoints = kwargs.get('checkpoints', "/home/mtiomoko/src/post_training_forecasting_official/checkpoints/")  # XXX
...
def define_model(train_loader, val_loader, test_loader, config):
    ...
    if config.get("model_file"):
        ...
        args = Args(
            task_name=config.get("task_name", "short_term_forecast"),
            model=config['models'],  # consistent with config key
            train_loader=None,
            val_loader=None,
            test_loader=None,
            ...
            exog_dim=config.get("exog_dim", 0),  # add exog_dim
            channel_id=config.get("channel_id"),  # add exog_dim
        )
        ...
        for batch_idx, (X_batch, y_batch, seq_x_mark_batch, seq_y_mark_batch) in enumerate(train_loader):
            dim = X_batch.shape[-1]  # safer way to get feature dim
            break
        ...
        args = Args(
            ...
            n_jobs=config['n_jobs'],
            feature_size=config.get("feature_size"),
            exog_dim=config.get("exog_dim", 0),
