...
from src.post_training.actions_definition import generate_random_params_for_action
...
def contextual_bandit_random(
    ...
    # print('SHAPE', pred.shape, true.shape, batch_x.shape)

    init_pred = copy.deepcopy(pred)
    n_channels = true.shape[2]
    sample_idx = 0  # for visualization
    mse_values = mean_squared_error(true.flatten(), pred.flatten())

    # print('Initial MSE:', mean_squared_error(pred.flatten(), true.flatten()))
    print('Initial MSE:', mean_squared_error(true.flatten(), pred.flatten()))
    ...
    for episode in range(episodes):
        ...
        for channel_idx in range(n_channels):
            ...
            # initial_mse = np.mean((chantrue - pred) ** 2, axis=(1, 2))
            ...
            for action in function_types:
                if action in ['log_transform', 'normalize_predictions', 'align_distribution']:
                    ...
                    for i in range(action_budget):
                        params = generate_random_parameters_function(action, batch_x)
                        # print(params)
                        # print(i, 'channel_pred', channel_pred[0, 0])
                        new_pred = generic_function_class(action, params).apply(channel_pred.copy().reshape(pred.shape[0], pred.shape[1], 1), channel_batch)
                        # print('comp21', channel_true[0, 21], pred[0, 21], new_pred[0, 21])
                        # print('comp22', channel_true[0, 22], pred[0, 22], new_pred[0, 22])
                        # print(channel_true.shape, new_pred.shape)
                        mse = mean_squared_error(channel_true.flatten(), new_pred.flatten())
                        action_performance.append((action, mse, params))
            ...
            # print('best_mse_channel', best_mse_channel)
        ...
        # total_mse = np.mean((true - pred) ** 2, axis=(1, 2))
    ...
    # print('Final MSE:', np.mean((true - pred) ** 2, axis=(1, 2)))

    if streamlit:
        final_plot(axes, fig, mse_history)
        save_predictions(batch_x, init_pred, true, best_pred, feedback)
    else:
